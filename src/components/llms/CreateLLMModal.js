...
const CreateLLMModal = ({ onClose, onSuccess }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    general_prompt: '',
    begin_message: '',
    start_speaker: 'agent', // 'agent' or 'user'
    general_tools: [
       {
      "type": "end_call",
      "name": "end_call",
  ...
  const handleToolToggle = (toolName) => {
    setFormData(prev => {
      ...
      if (toolExists) {
        ...
        // Map to backend structure: type, name, description
        const newTool = {
            type: toolDef.name, // Assuming type matches name for these basics, or strictly 'end_call'/'transfer_call'
            name: toolDef.name,
            description: toolDef.description
        };
        // Special case for transfer (mocking destination for now as per docs example possibility)
        if (toolName === 'transfer_call') {
            newTool.name = 'transfer_to_manager'; // Matching doc example? Or keep generic? Docs say: name: "transfer_to_manager". Let's verify docs.
            // Docs example: "name": "transfer_to_manager". 
            // Better to keep it simple for now or ask user, but let's stick to the example structure if possible.
            // Actually, let's just push the basic structure.
        }
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await llmService.createLLM(formData);
      onSuccess();
      onClose();
    } catch (error) {
      console.error('Error creating LLM:', error);
      alert('Failed to create LLM');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center p-6 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-800">Create New LLM</h2>
            ...
            <div className="flex space-x-4">
              <label className="flex items-center">
                <input
                  ...
                  name="start_speaker"
                  value="agent"
                  checked={formData.start_speaker === 'agent'}
                  onChange={handleChange}
                  className="mr-2"
                />
                Agent
              ...
              <label className="flex items-center">
                <input
                  ...
                  name="start_speaker"
                  value="user"
                  checked={formData.start_speaker === 'user'}
                  onChange={handleChange}
                  className="mr-2"
                />
                User
