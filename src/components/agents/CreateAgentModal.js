...
const CreateAgentModal = ({ onClose, onCreated, initialLLMData }) => {
  const [formData, setFormData] = useState({
    agent_name: initialLLMData?.agent_name || "",
    response_engine: {
      type: "retell-llm",
      llm_id: initialLLMData?.llm_id || "",
    },
        ...
        <div className="flex justify-between items-center p-6 border-b border-gray-200">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Configure your voice agent's brain and personality
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            <div>
              ...
              {llms.length === 0 && (
                <p className="text-sm text-red-500 mt-2">
                  No LLMs found. Please create an LLM first.
                </p>
              )}
              {formData.response_engine.llm_id &&
                !formData.response_engine.llm_id.startsWith("llm_") && (
                  <p className="text-xs text-amber-600 mt-1">
                    Warning: Selected LLM doest not appear to have a valid
                    Retell ID (should start with "llm_").
                  </p>
                )}
            </div>
