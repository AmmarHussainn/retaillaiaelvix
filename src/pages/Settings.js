...
const Settings = () => {
  ...
  const handleDeleteClick = (id) => {
    setConfirmationModal({ isOpen: true, keyId: id });
  };

  const handleConfirmDelete = async () => {
    const { keyId } = confirmationModal;
    if (!keyId) return;

    try {
      await settingsService.deleteApiKey(keyId);
      setApiKeys(apiKeys.filter(k => k.id !== keyId && k._id !== keyId));
      setMsg({ text: 'API Key deleted.', type: 'success' });
    } catch (err) {
      console.error('Delete key error:', err);
      setMsg({ text: 'Failed to delete API key.', type: 'error' });
    } finally {
      setConfirmationModal({ isOpen: false, keyId: null });
    }
  };

  const copyToClipboard = (text) => {
    navigator.clipboard.writeText(text);
    // Simple visual feedback could be improved, but reused existing alert replacement
    // For copy feedback we typically use a toast or small text change, let's assume valid toast or just no-op visual besides browser default
    // Or we could use setMsg for a quick flash
  ...
  return (
    <div className="space-y-6">
               ...
               <ul className="divide-y divide-gray-200">
                 {apiKeys.map((key) => (
                   <li key={key.id || key._id} className="px-6 py-4 flex items-center justify-between hover:bg-gray-50">
                     ...
                     <button
                       onClick={() => handleDeleteClick(key.id || key._id)}
      ...
      <ConfirmationModal
        ...
        onClose={() => setConfirmationModal({ isOpen: false, keyId: null })}
