                              ...
                              onClick={() => {
                                ...
                                if (type.type === 'transfer_call') {
                                  Object.assign(baseTool, {
                                    ...
                                    whisper_message: { type: 'static', content: '' },
                                    three_way_enabled: false,
                                    three_way_message: { type: 'static', content: '' },
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-900">
                    {activeConfigTool.type === 'extract_dynamic_variable' ? 'Extract Dynamic Variable' : 
                     activeConfigTool.type === 'custom' ? 'Custom Function' : 
                     activeConfigTool.type === 'transfer_call' ? 'Transfer Call' : 'Configure Tool'}
                  </h3>
            ...
            <div className="p-8 space-y-6 max-h-[75vh] overflow-y-auto custom-scrollbar">
              ...
              {activeConfigTool.type === 'transfer_call' && (
                <div className="space-y-8 pt-4">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <label className="text-xs font-bold text-gray-900 uppercase tracking-wider ml-1">Transfer to</label>
                      <div className="flex items-center space-x-6">
                        <div className="flex bg-gray-100 p-1 rounded-xl">
                          {['static', 'dynamic'].map((t) => (
                            <button 
                              key={t}
                              type="button"
                              onClick={() => setActiveConfigTool({ ...activeConfigTool, transfer_to_type: t })}
                              className={`px-4 py-1.5 rounded-lg text-[11px] font-bold transition-all ${activeConfigTool.transfer_to_type === t ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500 hover:text-gray-900'}`}
                            >
                        ...
                        <div className="flex items-center space-x-4">
                           <div className="flex items-center space-x-2">
                              <input 
                                ...
                                onChange={(e) => setActiveConfigTool({ ...activeConfigTool, ignore_e164_validation: e.target.checked })}
                                className="w-4 h-4 accent-gray-900 rounded"
                              />
                              <label htmlFor="ignore_e164" className="text-[11px] font-bold text-gray-700 cursor-pointer">Ignore E.164 Validation</label>
                           </div>
                        </div>
                      </div>
                    </div>
                    ...
                    {activeConfigTool.transfer_to_type === 'static' ? (
                      <div className="flex space-x-3 items-start">
                        <div className="flex-1 space-y-2">
                          <input 
                            ...
                            onChange={(e) => setActiveConfigTool({ ...activeConfigTool, transfer_to_number: e.target.value })}
                          ...
                          />
                          <p className="text-[11px] text-gray-500 font-medium ml-1">Enter a static phone number / SIP URI / dynamic variable.</p>
                        </div>
                        <div className="w-[120px] space-y-2">
                          <input 
                            value={activeConfigTool.extension_number || ''}
                            onChange={(e) => setActiveConfigTool({ ...activeConfigTool, extension_number: e.target.value })}
                      ...
                      <div className="space-y-2">
                        <textarea 
                          ...
                          onChange={(e) => setActiveConfigTool({ ...activeConfigTool, transfer_to_prompt: e.target.value })}
                        ...
                        />
                        <p className="text-[11px] text-gray-500 font-medium ml-1">Use a prompt to handle dynamic call transfer routing.</p>
                      </div>
                    )}
                  </div>
                  ...
                  {/* Type (Cold / Warm) */}
                  <div className="space-y-3">
                    <label className="text-xs font-bold text-gray-900 uppercase tracking-wider ml-1">Type</label>
                    <div className="grid grid-cols-2 gap-4">
                      {[
                        { id: 'cold_transfer', label: 'Cold Transfer', icon: Phone, color: 'text-blue-500', bg: 'bg-blue-50', desc: 'AI transfers the call to the next agent without a debrief.' },
                        { id: 'warm_transfer', label: 'Warm Transfer', icon: Bot, color: 'text-orange-500', bg: 'bg-orange-50', desc: 'AI provides a debrief to the next agent after transferring the call.' }
                      ].map((t) => (
                        <button 
                          key={t.id}
                          type="button"
                          onClick={() => setActiveConfigTool({ ...activeConfigTool, transfer_type: t.id })}
                          className={`p-6 rounded-[24px] border-2 transition-all text-left flex flex-col items-center text-center space-y-4 ${activeConfigTool.transfer_type === t.id ? 'border-gray-900 bg-white shadow-xl shadow-gray-100 scale-[1.02]' : 'border-gray-100 bg-gray-50/50 hover:border-gray-300'}`}
                        >
                          <div className={`w-12 h-12 ${t.bg} rounded-full flex items-center justify-center`}>
                            <t.icon className={`w-6 h-6 ${t.color}`} />
                          </div>
                          <div>
                            <p className="text-[15px] font-bold text-gray-900">{t.label}</p>
                            <p className="text-[11px] text-gray-500 font-medium leading-relaxed mt-1">{t.desc}</p>
                          </div>
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-8">
                     <div className="space-y-3">
                        <label className="text-xs font-bold text-gray-900 uppercase tracking-wider ml-1">SIP Transfer Method</label>
                        <div className="grid grid-cols-2 gap-2">
                           {['SIP REFER', 'SIP INVITE'].map(m => (
                              <button 
                                 key={m}
                                 type="button"
                                 onClick={() => setActiveConfigTool({ ...activeConfigTool, sip_transfer_method: m.toLowerCase().replace(' ', '_') })}
                                 className={`px-4 py-3.5 rounded-2xl border-2 flex items-center justify-between transition-all ${activeConfigTool.sip_transfer_method === m.toLowerCase().replace(' ', '_') ? 'border-gray-900 bg-white' : 'border-gray-100 bg-gray-50/30 hover:border-gray-200'}`}
                              >
                                 <span className="text-xs font-bold text-gray-800">{m}</span>
                                 <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${activeConfigTool.sip_transfer_method === m.toLowerCase().replace(' ', '_') ? 'border-gray-900' : 'border-gray-300'}`}>
                                    {activeConfigTool.sip_transfer_method === m.toLowerCase().replace(' ', '_') && <div className="w-2.5 h-2.5 bg-gray-900 rounded-full" />}
                                 </div>
                              </button>
                           ))}
                        </div>
                     ...
                     <div className="space-y-3">
                        <div className="flex items-center space-x-1.5 ml-1">
                           <label className="text-xs font-bold text-gray-900 uppercase tracking-wider">Displayed Caller ID</label>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                           {['agent', 'user'].map(c => (
                              <button 
                                 key={c}
                                 type="button"
                                 onClick={() => setActiveConfigTool({ ...activeConfigTool, displayed_caller_id: c })}
                                 className={`px-4 py-3.5 rounded-2xl border-2 flex items-center justify-between transition-all ${activeConfigTool.displayed_caller_id === c ? 'border-gray-900 bg-white' : 'border-gray-100 bg-gray-50/30 hover:border-gray-200'}`}
                              >
                                 <span className="text-xs font-bold text-gray-800">{c === 'agent' ? "Retell Agent's Number" : "User's Number"}</span>
                                 <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${activeConfigTool.displayed_caller_id === c ? 'border-gray-900' : 'border-gray-300'}`}>
                                    {activeConfigTool.displayed_caller_id === c && <div className="w-2.5 h-2.5 bg-gray-900 rounded-full" />}
                                 </div>
                              </button>
                           ))}
                        </div>
                  ...
                  <div className="space-y-3">
                    <div>
                      <label className="text-xs font-bold text-gray-900 uppercase tracking-wider ml-1">Custom SIP Headers</label>
                      <p className="text-[11px] text-gray-500 font-medium ml-1">Add key/value pairs for call routing, metadata, or carrier integration.</p>
                    </div>
                    <div className="space-y-2">
                          ...
                          <div key={i} className="flex items-center space-x-2 animate-in slide-in-from-left-2 duration-200">
                             <input 
                                ...
                                onChange={(e) => {
                                   ...
                                   list[i].key = e.target.value;
                                   setActiveConfigTool({ ...activeConfigTool, custom_sip_headers: list });
                                }}
                             ...
                             <button 
                                type="button"
                                onClick={() => setActiveConfigTool({ ...activeConfigTool, custom_sip_headers: activeConfigTool.custom_sip_headers.filter((_, idx) => idx !== i) })}
                                className="p-2 text-gray-300 hover:text-red-500 transition-colors"
                             >
                                <X className="w-4 h-4" />
                             </button>
                       ...
                       <button 
                          type="button"
                          onClick={() => setActiveConfigTool({ ...activeConfigTool, custom_sip_headers: [...(activeConfigTool.custom_sip_headers || []), { key: '', value: '' }] })}
                          className="flex items-center space-x-2 text-gray-900 hover:text-blue-600 bg-white border border-gray-200 px-4 py-2 rounded-xl font-bold text-sm transition-all"
                       >
                          <Plus className="w-4 h-4" />
                          <span>Add</span>
                  ...
                  {activeConfigTool.transfer_type === 'warm_transfer' && (
                    <div className="space-y-3 pt-4 border-t border-gray-100 animate-in fade-in slide-in-from-top-4 duration-300">
                       <label className="text-xs font-bold text-gray-900 uppercase tracking-wider ml-1">Transfer Settings</label>
                       <div className="p-6 border border-gray-100 rounded-[24px] space-y-8 bg-gray-50/30">
                          ...
                          <div className="space-y-4">
                             <div className="flex items-center space-x-3">
                                ...
                                <span className="text-sm font-bold text-gray-900">During Transfer Call</span>
                             </div>
                             <div className="pl-11 space-y-2">
                                <label className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">On-hold Music</label>
                                <div className="relative group">
                                   <select 
                                      value={activeConfigTool.on_hold_music}
                                      onChange={(e) => setActiveConfigTool({ ...activeConfigTool, on_hold_music: e.target.value })}
                          ...
                          <div className="space-y-6">
                             <div className="flex items-center space-x-3">
                                ...
                                <span className="text-sm font-bold text-gray-900">During Agent Connection</span>
                             </div>
                             <div className="pl-11 space-y-6">
                                <div className="space-y-4">
                                   <div className="flex items-center justify-between">
                                      <div className="flex items-center space-x-1">
                                         <span className="text-[11px] font-bold text-gray-700">Agent Detection Timeout</span>
                                         <Info className="w-3 h-3 text-gray-300" />
                                      </div>
                                      <span className="text-[11px] font-bold text-gray-900">{activeConfigTool.detection_timeout_ms / 1000}s</span>
                                   </div>
                                   <input 
                                      type="range"
                                      min="5000" max="120000" step="5000"
                                      value={activeConfigTool.detection_timeout_ms}
                                      onChange={(e) => setActiveConfigTool({ ...activeConfigTool, detection_timeout_ms: Number(e.target.value) })}
                                ...
                                <div className="space-y-4">
                                   <div className="flex items-center justify-between">
                                      <div>
                                         <p className="text-xs font-bold text-gray-900">Private Handoff Message (Whisper)</p>
                                         <p className="text-[10px] text-gray-500 font-medium">Spoken only to the agent receiving the transfer.</p>
                                      </div>
                                      <button 
                                         type="button"
                                         onClick={() => setActiveConfigTool({ ...activeConfigTool, whisper_enabled: !activeConfigTool.whisper_enabled })}
                                         className={`w-10 h-5 rounded-full relative transition-all duration-200 ${activeConfigTool.whisper_enabled ? 'bg-gray-900' : 'bg-gray-200'}`}
                                      >
                                         <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all duration-200 ${activeConfigTool.whisper_enabled ? 'left-6' : 'left-1'}`} />
                                      </button>
                                   </div>
                                   {activeConfigTool.whisper_enabled && (
                                      <textarea 
                                         value={activeConfigTool.whisper_message.content}
                                         onChange={(e) => setActiveConfigTool({ ...activeConfigTool, whisper_message: { ...activeConfigTool.whisper_message, content: e.target.value } })}
                                ...
                                <div className="space-y-4">
                                   <div className="flex items-center justify-between">
                                      <div>
                                         <p className="text-xs font-bold text-gray-900">Public Handoff Message (Three-way)</p>
                                         <p className="text-[10px] text-gray-500 font-medium">Spoken to both parties after the transfer is successful.</p>
                                      </div>
                                      <button 
                                         type="button"
                                         onClick={() => setActiveConfigTool({ ...activeConfigTool, three_way_enabled: !activeConfigTool.three_way_enabled })}
                                         className={`w-10 h-5 rounded-full relative transition-all duration-200 ${activeConfigTool.three_way_enabled ? 'bg-gray-900' : 'bg-gray-200'}`}
                                      >
                                         <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all duration-200 ${activeConfigTool.three_way_enabled ? 'left-6' : 'left-1'}`} />
                                      </button>
                                   </div>
                                   {activeConfigTool.three_way_enabled && (
                                      <textarea 
                                         value={activeConfigTool.three_way_message.content}
                                         onChange={(e) => setActiveConfigTool({ ...activeConfigTool, three_way_message: { ...activeConfigTool.three_way_message, content: e.target.value } })}
                                         className="w-full px-4 py-3 bg-white rounded-xl text-xs font-medium outline-none border border-gray-100 min-h-[80px] resize-none animate-in fade-in slide-in-from-top-2 duration-200"
                  ...
                  <div className="space-y-4 pt-4 border-t border-gray-100">
                    <div className="flex items-center space-x-3">
                      ...
                      <label htmlFor="speak_during" className="flex-1 cursor-pointer">
                        <p className="text-sm font-bold text-gray-900">Speak During Execution</p>
                        <p className="text-[11px] text-gray-500 font-medium">If enabled, the agent will say something while transferring the call.</p>
                      </label>
                    </div>
